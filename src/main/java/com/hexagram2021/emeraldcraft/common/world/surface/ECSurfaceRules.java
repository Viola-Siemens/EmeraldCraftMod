...
public class ECSurfaceRules {
	...
	public static SurfaceRules.RuleSource overworldLike(boolean checkAbovePreliminarySurface, boolean bedrockRoof, boolean bedrockFloor) {
		...
		SurfaceRules.ConditionSource isAzureSandstoneLiningBiome = SurfaceRules.isBiome(ECBiomeKeys.AZURE_DESERT);
		SurfaceRules.ConditionSource isJadeiteSandstoneLiningBiome = SurfaceRules.isBiome(ECBiomeKeys.JADEITE_DESERT);
		
		SurfaceRules.RuleSource hillAndDesertSource = SurfaceRules.sequence(
				SurfaceRules.ifTrue(
				...
				SurfaceRules.ifTrue(isAzureSandstoneLiningBiome, azureSandstoneLinedSand),
				SurfaceRules.ifTrue(isJadeiteSandstoneLiningBiome, jadeiteSandstoneLinedSand),
				SurfaceRules.ifTrue(
						SurfaceRules.isBiome(ECBiomeKeys.GOLDEN_BEACH),
						SurfaceRules.sequence(
								SurfaceRules.ifTrue(
										SurfaceRules.noiseCondition(Noises.SURFACE, 0.8D),
										SurfaceRules.sequence(SurfaceRules.ifTrue(SurfaceRules.ON_CEILING, RED_SANDSTONE), RED_SAND)
								),
								SurfaceRules.ifTrue(
										SurfaceRules.noiseCondition(Noises.SURFACE, 0.4D, 0.8D),
		...
		SurfaceRules.RuleSource smallPowderSnow = SurfaceRules.ifTrue(SurfaceRules.noiseCondition(Noises.POWDER_SNOW, 0.45D, 0.58D), POWDER_SNOW);
		SurfaceRules.RuleSource largePowderSnow = SurfaceRules.ifTrue(SurfaceRules.noiseCondition(Noises.POWDER_SNOW, 0.35D, 0.6D), POWDER_SNOW);
		SurfaceRules.RuleSource underSurfaceSource = SurfaceRules.sequence(
				...
				SurfaceRules.ifTrue(
						...
						SurfaceRules.sequence(smallPowderSnow, DIRT)
				),
				hillAndDesertSource,
				SurfaceRules.ifTrue(
		...
		SurfaceRules.RuleSource surfaceSource = SurfaceRules.sequence(
				...
				SurfaceRules.ifTrue(
						SurfaceRules.isBiome(Biomes.GROVE),
						SurfaceRules.sequence(largePowderSnow, SNOW_BLOCK)
				),
				hillAndDesertSource,
				SurfaceRules.ifTrue(
						SurfaceRules.isBiome(Biomes.WINDSWEPT_SAVANNA),
						SurfaceRules.sequence(
								SurfaceRules.ifTrue(surfaceNoiseAbove(1.75D), STONE), SurfaceRules.ifTrue(surfaceNoiseAbove(-0.5D), COARSE_DIRT)
						)
				),
				SurfaceRules.ifTrue(
						SurfaceRules.isBiome(Biomes.WINDSWEPT_GRAVELLY_HILLS),
				...
				SurfaceRules.ifTrue(
						...
						SurfaceRules.sequence(
								...
								SurfaceRules.ifTrue(
										...
										SurfaceRules.ifTrue(SurfaceRules.stoneDepthCheck(0, true, true, CaveSurface.FLOOR), SANDSTONE)
								),
								SurfaceRules.ifTrue(
										isAzureSandstoneLiningBiome,
										SurfaceRules.ifTrue(SurfaceRules.stoneDepthCheck(0, true, true, CaveSurface.FLOOR), AZURE_SANDSTONE)
								),
								SurfaceRules.ifTrue(
										isJadeiteSandstoneLiningBiome,
										SurfaceRules.ifTrue(SurfaceRules.stoneDepthCheck(0, true, true, CaveSurface.FLOOR), JADEITE_SANDSTONE)
								)
						)
				),
				SurfaceRules.ifTrue(
		...
		SurfaceRules.RuleSource abovePreliminarySurface = SurfaceRules.ifTrue(SurfaceRules.abovePreliminarySurface(), ruleSource);
		builder.add(checkAbovePreliminarySurface ? abovePreliminarySurface : ruleSource);
