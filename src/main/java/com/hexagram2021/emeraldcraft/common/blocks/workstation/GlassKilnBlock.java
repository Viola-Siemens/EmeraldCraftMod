...
public class GlassKilnBlock extends AbstractFurnaceBlock {
	...
	public GlassKilnBlock(AbstractBlock.Properties properties) {
		super(properties);
	}

	@Nonnull
	public ActionResultType use(@Nonnull BlockState blockState, World level, @Nonnull BlockPos blockPos,
								@Nonnull PlayerEntity player, @Nonnull Hand interactionHand, @Nonnull BlockRayTraceResult blockHitResult) {
	...
	public TileEntity newBlockEntity(@Nonnull IBlockReader level) {
		return new GlassKilnBlockEntity();
	}
	...
	public void setPlacedBy(@Nonnull World level, @Nonnull BlockPos blockPos, @Nonnull BlockState blockState, @Nonnull LivingEntity livingEntity, ItemStack itemStack) {
		if (itemStack.hasCustomHoverName()) {
			...
			if (blockentity instanceof GlassKilnBlockEntity) {
				((GlassKilnBlockEntity)blockentity).setCustomName(itemStack.getHoverName());
			}
		}
	}
	...
	public void onRemove(BlockState blockState, @Nonnull World level, @Nonnull BlockPos blockPos, BlockState newBlockState, boolean b) {
		if (!blockState.is(newBlockState.getBlock())) {
			TileEntity blockentity = level.getBlockEntity(blockPos);
			if (blockentity instanceof GlassKilnBlockEntity) {
				GlassKilnBlockEntity glassKilnBlockEntity = (GlassKilnBlockEntity)blockentity;
				if (level instanceof ServerWorld) {
					ServerWorld serverLevel = (ServerWorld)level;
					InventoryHelper.dropContents(serverLevel, blockPos, glassKilnBlockEntity);
					glassKilnBlockEntity.getRecipesToAwardAndPopExperience(serverLevel, Vector3d.atCenterOf(blockPos));
				}
