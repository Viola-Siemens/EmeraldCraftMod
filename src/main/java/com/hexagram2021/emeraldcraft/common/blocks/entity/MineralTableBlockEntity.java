...
public class MineralTableBlockEntity extends LockableTileEntity implements ISidedInventory, IRecipeHolder, IRecipeHelperPopulator, ITickableTileEntity {
	...
	protected final IIntArray dataAccess = new IIntArray() {
		public int get(int index) {
			switch (index) {
				case DATA_LIT_TIME:
					return MineralTableBlockEntity.this.litTime;
				case DATA_LIT_DURATION:
					return MineralTableBlockEntity.this.litDuration;
				case DATA_COOKING_PROGRESS:
					return MineralTableBlockEntity.this.cookingProgress;
				case DATA_COOKING_TOTAL_TIME:
					return MineralTableBlockEntity.this.cookingTotalTime;
				default:
					return 0;
			}
		}
		...
		public void set(int index, int value) {
			switch (index) {
				case DATA_LIT_TIME:
					MineralTableBlockEntity.this.litTime = value;
					break;
				case DATA_LIT_DURATION:
					MineralTableBlockEntity.this.litDuration = value;
					break;
				case DATA_COOKING_PROGRESS:
					MineralTableBlockEntity.this.cookingProgress = value;
					break;
				case DATA_COOKING_TOTAL_TIME:
					MineralTableBlockEntity.this.cookingTotalTime = value;
					break;
			}
	...
	private final IRecipeType<MineralTableRecipe> recipeType;
	
	public MineralTableBlockEntity() {
		super(ECBlockEntity.MINERAL_TABLE.get());
		this.recipeType = ECRecipes.MINERAL_TABLE_TYPE;
	}

	@Override @Nonnull
	protected TranslationTextComponent getDefaultName() {
		return new TranslationTextComponent("container.mineral_table");
	}
	...
	public void tick() {
		...
		if (this.isLit() || !itemstack.isEmpty() && !this.items.get(SLOT_INPUT).isEmpty()) {
			MineralTableRecipe recipe = this.level.getRecipeManager().getRecipeFor(this.recipeType, this, this.level).orElse(null);
		...
		if (flag != this.isLit()) {
			flag1 = true;
			BlockState blockState = this.level.getBlockState(this.worldPosition).setValue(MineralTableBlock.LIT, this.isLit());
	...
	protected int getBurnDuration(ItemStack itemStack) {
		...
		return itemStack.getItem() == Items.BLAZE_POWDER ? MineralTableRecipe.BURN_TIME * 20 : 0;
	}
	...
	private static int getTotalCookTime(World level, IRecipeType<MineralTableRecipe> recipeType, IInventory container) {
