...
import cn.sh1rocu.emeraldcraft.util.BlockStateUtil;
...
public interface RegistryChecker {
    ...
    static void registryCheck(LootDataManager lootDataManager) {
        BuiltInRegistries.BLOCK.forEach(block -> {
            ...
            //c tags
            tagCheckSuffix(id, block, blockItem, "_ore", ConventionalBlockTags.ORES, ConventionalItemTags.ORES).ifFailed(
                    () -> tagCheckPrefix(id, block, blockItem, "ore_", ConventionalBlockTags.ORES, ConventionalItemTags.ORES)
            );
    ...
    static void recipeCheck(LootDataManager lootDataManager, RecipeManager recipeManager, RegistryAccess registryAccess) {
            ...
            if (!entityType.getDefaultLootTable().equals(BuiltInLootTables.EMPTY)) {
                ...
                if (!table.equals(LootTable.EMPTY)) {
                    for (LootPool pool : table.pools) {
                        Arrays.stream(pool.entries).forEach(entry -> {
        ...
        BuiltInRegistries.ITEM.forEach(item -> {
            ...
            if (item instanceof BlockItem blockItem) {
                ...
                if ((blockState = BlockStateUtil.getAxeStrippingState(block.defaultBlockState())) != null) {
                    graph.addEdge(item, blockState.getBlock().asItem());
                }
                if ((blockState = BlockStateUtil.getShovelPathingState(block.defaultBlockState())) != null) {
                    graph.addEdge(item, blockState.getBlock().asItem());
                }
    ...
    private static void addEdgesForLoot(Item item, ItemGraph graph, LootDataManager lootDataManager) {
        ...
        if (item instanceof BlockItem blockItem) {
            ...
            if (!block.getLootTable().equals(BuiltInLootTables.EMPTY)) {
                ...
                if (!table.equals(LootTable.EMPTY)) {
                    for (LootPool pool : table.pools) {
                        Arrays.stream(pool.entries).forEach(entry -> addEdgeForLootTableEntry(item, entry, graph, lootDataManager));
                    }
                }
            }
        }
