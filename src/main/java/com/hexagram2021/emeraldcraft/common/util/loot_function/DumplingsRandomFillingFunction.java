...
public class DumplingsRandomFillingFunction extends LootItemConditionalFunction {
    ...
    protected DumplingsRandomFillingFunction(LootItemCondition[] predicates, Optional<List<Item>> fillings) {
        super(predicates);
    ...
    public static class Serializer extends LootItemConditionalFunction.Serializer<DumplingsRandomFillingFunction> {
        public void serialize(JsonObject json, DumplingsRandomFillingFunction function, JsonSerializationContext context) {
            super.serialize(json, function, context);

            function.fillings.ifPresent(items -> {
                JsonArray array = new JsonArray();
                items.forEach(item -> array.add(getRegistryName(item).toString()));
                json.add("fillings", array);
            });
        }

        public DumplingsRandomFillingFunction deserialize(JsonObject json, JsonDeserializationContext context, LootItemCondition[] conditions) {
            Optional<List<Item>> fillings;
            JsonArray array = GsonHelper.getAsJsonArray(json, "fillings", null);
            if (array == null) {
                fillings = Optional.empty();
            } else {
                ...
                fillings = Optional.of(items);
            }
            return new DumplingsRandomFillingFunction(conditions, fillings);
        }
    }
}
