...
public record TradeShadowRecipe(ResourceLocation id, ItemStack costA, ItemStack costB, ItemStack result,
                                EntityType<?> entityType,
                                @Nullable VillagerProfession profession, int villagerLevel,
                                int xp) implements Recipe<Container> {
    ...
    public static Map<VillagerProfession, List<Block>> getAllJobsites() {
        ...
        if (ECCommonConfig.ENABLE_JEI_TRADING_SHADOW_RECIPE.get()) {
            BuiltInRegistries.VILLAGER_PROFESSION.forEach(villagerProfession -> {
                ...
                BuiltInRegistries.POINT_OF_INTEREST_TYPE.forEach(poiType -> {
                    if (villagerProfession.heldJobSite().test(BuiltInRegistries.POINT_OF_INTEREST_TYPE.wrapAsHolder(poiType))) {
    ...
    public RecipeSerializer<?> getSerializer() {
        return ECRecipeSerializer.TRADE_SHADOW_SERIALIZER;
    }

    @Override
    public RecipeType<?> getType() {
        return ECRecipes.TRADE_SHADOW_TYPE;
    }
