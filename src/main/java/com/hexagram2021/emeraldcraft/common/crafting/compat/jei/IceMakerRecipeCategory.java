...
public class IceMakerRecipeCategory implements IRecipeCategory<IceMakerRecipe> {
	...
	public String getTitle() {
		return new TranslationTextComponent("block.emeraldcraft.ice_maker").getString();
	}
	...
	@Override
	public void draw(IceMakerRecipe recipe, MatrixStack poseStack, double mouseX, double mouseY) {
	...
	protected void drawCookTime(IceMakerRecipe recipe, MatrixStack poseStack, int y) {
		int freezeTime = recipe.getFreezingTime();
		if (freezeTime > 0) {
			int cookTimeSeconds = freezeTime / 20;
			TranslationTextComponent timeString = new TranslationTextComponent("gui.emeraldcraft.ice_maker.time.seconds", cookTimeSeconds);
