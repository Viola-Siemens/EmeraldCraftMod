...
public record SuspiciousStewCookstoveRecipe(ResourceLocation id, FluidStack fluidStack, Ingredient container,
                                            ItemStack result,
                                            ICookstoveDisplay display, int cookTime) implements ICookstoveRecipe {
    ...
    public ItemStack assemble(CookstoveBlockEntity container, RegistryAccess registryAccess) {
        ...
        for (int i = 0; i < container.getContainerSize(); ++i) {
            ...
            if (!ingredient.isEmpty()) {
                ...
                if (effect != null) {
                    SuspiciousStewItem.saveMobEffect(result, effect.getSuspiciousEffect(), effect.getEffectDuration());
                    break;
                }
            }
        }
    ...
    public SuspiciousStewCookstoveRecipeSerializer<SuspiciousStewCookstoveRecipe> getSerializer() {
        return ECRecipeSerializer.SUSPICIOUS_STEW_COOKSTOVE_SERIALIZER;
    }

    @Override
    public RecipeType<SuspiciousStewCookstoveRecipe> getType() {
        return ECRecipes.SUSPICIOUS_STEW_COOKSTOVE_TYPE;
    }
}
