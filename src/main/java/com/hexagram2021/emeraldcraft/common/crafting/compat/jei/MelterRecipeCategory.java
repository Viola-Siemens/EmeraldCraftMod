...
public class MelterRecipeCategory implements IRecipeCategory<MelterRecipe> {
	...
	public MelterRecipeCategory(IGuiHelper guiHelper) {
		...
		this.icon = guiHelper.createDrawableIngredient(new ItemStack(ECBlocks.WorkStation.MELTER));
	...
	public String getTitle() {
		return new TranslationTextComponent("block.emeraldcraft.melter").getString();
	}
	...
	public void setIngredients(MelterRecipe recipe, IIngredients ingredients) {
		ingredients.setInput(VanillaTypes.ITEM, recipe.getIngredient().getItems()[0]);
	}

	@Override
	public void draw(MelterRecipe recipe, MatrixStack poseStack, double mouseX, double mouseY) {
	...
	protected void drawCookTime(MelterRecipe recipe, MatrixStack poseStack, int y) {
		int meltTime = recipe.getMeltingTime();
		if (meltTime > 0) {
			int cookTimeSeconds = meltTime / 20;
			TranslationTextComponent timeString = new TranslationTextComponent("gui.emeraldcraft.melter.time.seconds", cookTimeSeconds);
	...
	public void setRecipe(IRecipeLayout recipeLayout, MelterRecipe recipe, IIngredients ingredients) {
		...
		itemStacks.init(MelterMenu.INGREDIENT_SLOT, true, 0, 0);
