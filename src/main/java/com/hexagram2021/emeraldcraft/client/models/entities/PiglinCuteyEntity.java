...
public class PiglinCuteyEntity extends AbstractVillager implements PiglinCuteyDataHolder {
	...
	private int sayGoodbyeStatus;

	public PiglinCuteyEntity(EntityType<? extends AbstractVillager> entityType, Level level) {
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new PiglinCuteyEntity.RushToPortalGoal(this, 2.0D, SPEED_MODIFIER));
		this.goalSelector.addGoal(1, new PiglinCuteyEntity.BackToPlayerGoal(this, 2.0D, 20.0D, SPEED_MODIFIER * 0.5D));
		this.goalSelector.addGoal(1, new PiglinCuteyEntity.BackToPortalGoal(this, SPEED_MODIFIER * 0.5D));
	...
	public int getPlayerReputation(Player player) {
		return (int)Math.floor(foodLevel / (MULTIPLIER_FOOD_THRESHOLD * 0.05D));
	}
	...
	class RushToPortalGoal extends Goal {
		...
		RushToPortalGoal(PiglinCuteyEntity cutey, double stopDistance, double speedModifier) {
			this.cutey = cutey;
		...
		@Override
		public void stop() {
			ECLogger.debug("piglin_cutey: status = 1");
			this.cutey.sayGoodbyeStatus = 1;
			PiglinCuteyEntity.this.navigation.stop();
		}
		...
		public boolean canUse() {
			...
			return this.cutey.sayGoodbyeStatus < 1 && blockpos != null && this.isTooFarAway(blockpos, this.stopDistance);
		}

		protected int nextStartTick(PathfinderMob p_25618_) { return INTERVAL_TICKS + p_25618_.getRandom().nextInt(INTERVAL_TICKS); }

		@Override
		public boolean canContinueToUse() {
			return this.cutey.sayGoodbyeStatus < 1;
		}
	...
	class BackToPlayerGoal extends Goal {
		final PiglinCuteyEntity cutey;
		final double stopDistance;
		final double giveupDistance;
		final double speedModifier;

		BackToPlayerGoal(PiglinCuteyEntity cutey, double stopDistance, double giveupDistance, double speedModifier) {
			this.cutey = cutey;
			this.stopDistance = stopDistance;
		...
		@Override
		public void stop() {
			ECLogger.debug("piglin_cutey: status = 2");
			this.cutey.sayGoodbyeStatus = 2;
			PiglinCuteyEntity.this.navigation.stop();
		...
		public boolean canUse() {
			return this.cutey.sayGoodbyeStatus == 1 &&
					this.cutey.lastTradedPlayer != null &&
					this.isTooFarAway(this.cutey.lastTradedPlayer, this.giveupDistance);
		}

		@Override
		public boolean canContinueToUse() {
			return this.cutey.sayGoodbyeStatus < 2;
		}
		...
		public void tick() {
			if(this.cutey.lastTradedPlayer == null || isTooFarAway(this.cutey.lastTradedPlayer, this.giveupDistance)) {
				this.stop();
			} else if (PiglinCuteyEntity.this.navigation.isDone()) {
				PiglinCuteyEntity.this.navigation.moveTo(this.cutey.lastTradedPlayer, this.speedModifier);
			}
		}

		private boolean isTooFarAway(LivingEntity entity, double dist) {
			return !entity.closerThan(this.cutey, dist);
		}
	}

	class BackToPortalGoal extends Goal {
		final PiglinCuteyEntity cutey;
		final double speedModifier;

		BackToPortalGoal(PiglinCuteyEntity cutey, double speedModifier) {
			this.cutey = cutey;
			this.speedModifier = speedModifier;
		}
		...
		public void tick() {
			BlockPos blockpos = this.cutey.getPortalTarget();
			if (PiglinCuteyEntity.this.navigation.isDone()) {
				PiglinCuteyEntity.this.navigation.moveTo(blockpos.getX(), blockpos.getY(), blockpos.getZ(), this.speedModifier);
			}
		}
		...
		@Override
		public void stop() {
			ECLogger.debug("piglin_cutey: status = 3");
			PiglinCuteyEntity.this.navigation.stop();
		}

		@Override
		public boolean canUse() {
			return this.cutey.sayGoodbyeStatus > 1 && this.cutey.getPortalTarget() != null;
		}
	}
