...
public class CookstoveRenderer implements BlockEntityRenderer<CookstoveBlockEntity> {
        ...
        if (level != null) {
            ...
            if (display == null) {
                ...
                if (!fluidVariant.isBlank()) {
                    ...
                    Material texture = new Material(InventoryMenu.BLOCK_ATLAS, FluidVariantRendering.getSprite(fluidVariant).atlasLocation());
                    int tintColor = FluidVariantRendering.getColor(fluidVariant, level, blockEntity.getBlockPos());
    ...
    private static void renderFluid(PoseStack transform, VertexConsumer builder, long amount, Material texture, int rgb, int tintColor) {
