	...
	public void render(CookstoveBlockEntity blockEntity, float partialTick, PoseStack transform, MultiBufferSource buffer, int rgb, int overlay) {
		if (blockEntity.getLevel() != null) {
			...
			for (int i = 0; i < CookstoveBlockEntity.COUNT_SLOTS; ++i) {
				...
				double animateTick = blockEntity.animateTick * 4.0D / 3.0D / SharedConstants.TICKS_PER_SECOND;
				double deltaHeight = Math.max(0.0D, Math.sin(animateTick) * 0.5D - 0.3D) * Math.pow(Math.sin(animateTick), 15.0D);
				double deltaRot = deltaHeight > 0.0D ? Math.sin(4.0D * animateTick) : 0.0D;
				transform.pushPose();
				transform.translate(dx + 0.5D, 0.7F + deltaHeight, dz + 0.5D);
				transform.rotateAround(Axis.XP.rotationDegrees(90.0F + (float) deltaRot), 0.0F, 0.0F, 0.0F);
