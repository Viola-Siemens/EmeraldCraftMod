...
public class ContinuousMinerScreen extends ContainerScreen<ContinuousMinerMenu> {
	...
	protected void renderTooltip(@Nonnull MatrixStack transform, int x, int y) {
		...
		if(this.hoveredSlot == null && this.isHovering(119, 20, 12, 49, x, y) && this.menu.getFluidLevel() > 0) {
			this.renderComponentTooltip(transform, this.getFluidTypeToolTips(this.menu.getFluidLevel()), x, y);
		}
	}
	
	private List<ITextComponent> getFluidTypeToolTips(int fluidLevel) {
		List<ITextComponent> ret = Lists.newArrayList(new TranslationTextComponent(FluidTypes.melted_emerald.getTranslationTag()));
		if(this.minecraft != null && this.minecraft.options.advancedItemTooltips) {
			ret.add(new TranslationTextComponent("fluids.save.bucket", String.format("%.2f", fluidLevel / 100.0F), new TranslationTextComponent(ECItems.MELTED_EMERALD_BUCKET.get().getDescriptionId())));
		}
		return ret;
	}
}
