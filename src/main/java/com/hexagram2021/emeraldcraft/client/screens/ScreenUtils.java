...
public final class ScreenUtils {
    ...
    public static void renderFluid(GuiGraphics transform, FluidStack fluidStack, int tankX, int tankY, int fluidWidth, int maxTankHeight, long maxTankCapability) {
        ...
        if (sprite == null)
            return;
        int tintColor = FluidVariantRendering.getColor(fluidVariant);
        float a = ((tintColor >> 24) & 0xff) / 255.0F;
        float r = ((tintColor >> 16) & 0xff) / 255.0F;
        float g = ((tintColor >> 8) & 0xff) / 255.0F;
        float b = (tintColor & 0xff) / 255.0F;
        transform.setColor(r, g, b, a);
        int remaining = (int) Mth.clamp((float) ((maxTankCapability - fluidStack.getAmount()) * maxTankHeight) / maxTankCapability, 0, maxTankHeight - 1);
    ...
    public static void renderFluidFromContainer(GuiGraphics transform, Container container, int tankIndex, int tankX, int tankY, int fluidWidth, int maxTankHeight, long maxTankCapability) {
    ...
    public static void handleFluidSyncPacket(String containerType, List<FluidStack> fluidStacks) {
        Player player = Minecraft.getInstance().player;
        if (player != null && player.containerMenu instanceof IFluidSyncMenu menu) {
            if (menu.getContainerType().equals(containerType) && menu.getContainer() instanceof ISynchronizableContainer container) {
